...
// ─────────────────────────────────────────────────────────────────
// Cowboy Bebop Icons — jazz notes / space / bounty hunter
// ─────────────────────────────────────────────────────────────────

const CBPlay: React.FC<{ className?: string }> = ({ className }) => (
    <svg viewBox="0 0 24 24" fill="currentColor" className={cn("size-[1em]", className)}>
        {/* Jazz-point play triangle */}
        <path d="M6 3.5 L20 12 L6 20.5 Z" />
        {/* Eighth note flag accent */}
        <circle cx="6" cy="12" r="1.3" fill="currentColor" opacity="0.35" />
        <line x1="6" y1="5.5" x2="9" y2="4" stroke="currentColor" strokeWidth="1" opacity="0.4" />
    </svg>
)

const CBPause: React.FC<{ className?: string }> = ({ className }) => (
    <svg viewBox="0 0 24 24" fill="currentColor" className={cn("size-[1em]", className)}>
        {/* Jazz bar double lines */}
        <rect x="5" y="3" width="4" height="18" rx="1" />
        <rect x="15" y="3" width="4" height="18" rx="1" />
        {/* Small star accent */}
        <circle cx="12" cy="3" r="1" opacity="0.35" />
    </svg>
)

const CBVolumeHigh: React.FC<{ className?: string }> = ({ className }) => (
    <svg viewBox="0 0 24 24" fill="currentColor" className={cn("size-[1em]", className)}>
        <path d="M3 9v6h4l5 5V4L7 9H3z" />
        {/* Trumpet-style wavy jazz waves */}
        <path d="M16.5 7.5 C18.5 9.5 18.5 14.5 16.5 16.5" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
        <path d="M19 5 C22 8 22 16 19 19" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
    </svg>
)

const CBVolumeMid: React.FC<{ className?: string }> = ({ className }) => (
    <svg viewBox="0 0 24 24" fill="currentColor" className={cn("size-[1em]", className)}>
        <path d="M3 9v6h4l5 5V4L7 9H3z" />
        <path d="M16.5 7.5 C18.5 9.5 18.5 14.5 16.5 16.5" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
    </svg>
...
const CBVolumeLow: React.FC<{ className?: string }> = ({ className }) => (
    <svg viewBox="0 0 24 24" fill="currentColor" className={cn("size-[1em]", className)}>
        <path d="M3 9v6h4l5 5V4L7 9H3z" />
    </svg>
)

const CBVolumeMuted: React.FC<{ className?: string }> = ({ className }) => (
    <svg viewBox="0 0 24 24" fill="currentColor" className={cn("size-[1em]", className)}>
        <path d="M3 9v6h4l5 5V4L7 9H3z" />
        <line x1="16" y1="9" x2="22" y2="15" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
        <line x1="22" y1="9" x2="16" y2="15" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
...
const CBFullscreenEnter: React.FC<{ className?: string }> = ({ className }) => (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={cn("size-[1em]", className)}>
        {/* Space porthole expanding — corner arrows */}
        <polyline points="15 3 21 3 21 9" />
        <polyline points="9 21 3 21 3 15" />
        <line x1="21" y1="3" x2="14" y2="10" />
        <line x1="3" y1="21" x2="10" y2="14" />
        {/* Star at center */}
        <circle cx="12" cy="12" r="1.2" fill="currentColor" stroke="none" opacity="0.45" />
    </svg>
)

const CBFullscreenExit: React.FC<{ className?: string }> = ({ className }) => (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={cn("size-[1em]", className)}>
...
const CBSkipForward: React.FC<{ className?: string }> = ({ className }) => (
    <svg viewBox="0 0 24 24" fill="currentColor" className={cn("size-[1em]", className)}>
        <polygon points="5 4 15 12 5 20" />
        <rect x="16" y="4" width="3" height="16" rx="1" />
        {/* Comet dot accent */}
        <circle cx="18" cy="4.5" r="0.9" opacity="0.4" />
    </svg>
)

const CBSkipBack: React.FC<{ className?: string }> = ({ className }) => (
    <svg viewBox="0 0 24 24" fill="currentColor" className={cn("size-[1em]", className)}>
        <polygon points="19 4 9 12 19 20" />
        <rect x="5" y="4" width="3" height="16" rx="1" />
        <circle cx="6" cy="4.5" r="0.9" opacity="0.4" />
    </svg>
)

const CBPip: React.FC<{ className?: string }> = ({ className }) => (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={cn("size-[1em]", className)}>
        <rect x="2" y="4" width="20" height="14" rx="2" />
        <rect x="12" y="10" width="8" height="6" rx="1" fill="currentColor" fillOpacity="0.25" />
    </svg>
)

const CBPipOff: React.FC<{ className?: string }> = ({ className }) => (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={cn("size-[1em]", className)}>
        <rect x="2" y="4" width="20" height="14" rx="2" />
        <line x1="2" y1="4" x2="22" y2="18" />
    </svg>
)

export const cowboyBebopPlayerIcons: PlayerIconOverrides = {
    play: CBPlay,
    pause: CBPause,
    volumeHigh: CBVolumeHigh,
    volumeMid: CBVolumeMid,
    volumeLow: CBVolumeLow,
    volumeMuted: CBVolumeMuted,
    fullscreenEnter: CBFullscreenEnter,
    fullscreenExit: CBFullscreenExit,
    skipForward: CBSkipForward,
    skipBack: CBSkipBack,
    pip: CBPip,
    pipOff: CBPipOff,
}
