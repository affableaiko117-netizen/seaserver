...
// Simple broad queries without atleast filter for very obscure titles
const fallbacks = [
    { id: "pluto",               q: "robot android futuristic" },
    { id: "blade-of-the-immortal", q: "samurai sword feudal" },
    { id: "holyland",            q: "boxing martial arts street" },
]
...
for (const t of fallbacks) {
    const p = new URLSearchParams({ q: t.q, categories: "111", purity: "100", sorting: "views", order: "desc" })
    const j = await get("https://wallhaven.cc/api/v1/search?" + p.toString())
    if (!j.data?.length) { console.log(`${t.id}: 0 results`); continue }
